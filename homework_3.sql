...
SELECT s.artist_id, COUNT(s.id) AS song_count
FROM song s
WHERE s.artist_id IS NOT null -- filter out the songs where the artist is unknown
GROUP BY s.artist_id

-- Calculate the count of all songs per artist in the system for first 100 albums (ID < 100)
-- All albums have an ID < 100 (ID <= 50), so ID < 25 is used
SELECT s.artist_id, COUNT(s.id) AS song_count
FROM song s 
WHERE s.album_id < 25 AND s.artist_id IS NOT null -- filter out the songs where the artist is unknown
GROUP BY s.artist_id

-- Find the maximal duration and the average duration per song for each artist
SELECT s.artist_id, AVG(s.duration) AS avg_song_duration, MAX(s.duration) AS max_song_duration
FROM song s
WHERE s.artist_id IS NOT null -- filter out the songs where the artist is unknown
GROUP BY s.artist_id
...
WHERE s.artist_id IS NOT null -- filter out the songs where the artist is unknown
GROUP BY s.artist_id
HAVING MAX(s.duration) > (SELECT AVG(song.duration) FROM song)
