...
CREATE OR REPLACE FUNCTION get_artist_details(input_artist_id INTEGER)
RETURNS TABLE (
	date_of_birth DATE,
    full_name VARCHAR,
    country VARCHAR,
...
$$
DECLARE 
	row_artist_details artist_details%ROWTYPE;
	rad artist_details%ROWTYPE;
BEGIN
	SELECT *
	INTO row_artist_details
	FROM artist_details ad
	WHERE ad.artist_id = input_artist_id;
	
	rad = row_artist_details;
	
	RETURN QUERY 
	SELECT rad.date_of_birth, rad.full_name, rad.country, rad.city, rad.is_married, COALESCE(rad.spouse_name, 'Not existent');
END;
$$;

SELECT * FROM get_artist_details(1)
...
$$
DECLARE 
	row_artist_details RECORD;
	rad RECORD;
BEGIN
	SELECT ad.date_of_birth, ad.full_name, ad.country, ad.city, ad.is_married, ad.spouse_name
	INTO row_artist_details
	FROM artist_details ad
	WHERE ad.artist_id = input_artist_id;
...
CREATE OR REPLACE FUNCTION get_rock_songs(input_artist_id INTEGER)
RETURNS TABLE (
	song_name VARCHAR,
	date_of_birth DATE,
    full_name VARCHAR,
...
DECLARE
	RETURN QUERY
	SELECT DISTINCT s.name AS song_name, ad.date_of_birth, ad.full_name, ad.country, ad.city, ad.is_married, COALESCE(ad.spouse_name, 'Not existent')
	FROM song s
	INNER JOIN songs_genres sg ON s.id = sg.song_id
	INNER JOIN artist_details ad ON ad.artist_id = s.artist_id
	WHERE sg.genre_id = 3 AND s.artist_id = input_artist_id	
BEGIN
END;
$$;
...
CREATE OR REPLACE FUNCTION get_songs(input_album_id INTEGER)
RETURNS INTEGER
LANGUAGE PLPGSQL
AS
$$
...
BEGIN
	...
	WHERE al.id = input_album_id;
	
	IF song_count > 5 THEN
	RETURN song_count;
	ELSE RAISE NOTICE 'This album has less than 5 songs.';
	END IF;
END;
$$;
...
CREATE OR REPLACE FUNCTION get_artists(input_playlist_id INTEGER)
RETURNS TABLE (song_name VARCHAR, artist_name VARCHAR)
LANGUAGE PLPGSQL
AS
$$
BEGIN
	...
	WHERE ps.playlist_id = input_playlist_id;
END;
$$;

SELECT * FROM get_artists(1)
